...
$publish = Join-Path $dist 'grevconnect-app'
$downloadDir = Join-Path $dist 'grevconnect-downloads'
...
if ($LASTEXITCODE -ne 0) { throw 'GrevConnect publish failed.' }

$appExe = Join-Path $publish 'GrevConnect.exe'
if (-not (Test-Path $appExe)) {
    throw 'GrevConnect.exe was not produced by dotnet publish.'
}
...
if (-not $iscc) {
    throw 'Inno Setup 6 was not found. Install it before building the GrevConnect installer.'
}

$iss = Join-Path $root 'installer\GrevConnect.iss'
Write-Host 'Building GrevConnect single EXE installer...'
...
Write-Host "GrevConnect installer: $installer"
